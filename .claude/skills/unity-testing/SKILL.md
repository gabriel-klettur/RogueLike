---
name: unity-testing
description: Valkur Unity test conventions — layered layout (root per production layer, one assembly each, enforced by TestLayoutConventionTests), categories, TestReflection, EditMode gotchas (TMP NRE, Unity null vs C# null, `renderer.material` leaks, `LogAssert.ignoreFailingMessages`), test template, MCP run/poll pattern, namespace enforcement script. Load before creating, fixing, or reorganizing tests under `unity/Valkur/Assets/Tests/`.
---
