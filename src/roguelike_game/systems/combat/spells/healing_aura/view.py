...
class HealingAuraView:
    ...
    def render(self, screen, camera):
        ...
        sprite_w = model.player.sprite_size[0]
        base_x = model.player.x + sprite_w / 2
        base_y = model.player.y + 96  # Altura de pies
        world_pos = camera.apply((base_x, base_y))

        ellipse_width, _ = camera.scale((sprite_w, 1))
        ellipse_height = int(ellipse_width * 0.3)
        alpha = max(0, 255 * (1 - model.timer / model.elipse_lifespan))
