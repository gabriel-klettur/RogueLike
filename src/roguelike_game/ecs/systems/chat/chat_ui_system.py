...
class ChatUISystem:
    ...
    def update(self, world, screen, camera):
        ...
        rh_size = btn_size
        rh_rect = pygame.Rect(btn_x - rh_size - 4, btn_y, rh_size, rh_size)
        try:
        ...
        if total_lines > 0:
            ...
            thumb_h = max(20, int(sb_rect.h * frac))
            if max_scroll > 0:
                pos_frac = scroll / float(max_scroll)
            else:
                pos_frac = 0.0
            thumb_y = sb_rect.y + int((sb_rect.h - thumb_h) * pos_frac)
        ...
        if ev.type == pygame.MOUSEWHEEL:
            ...
            if resize_rect and resize_rect.collidepoint(mx, my):
                ...
                if sb_rect.h - thumb_h > 0:
                    ...
                    pos_frac = 0.0
                state.chat_scroll_lines = int(round(max_scroll * pos_frac))
