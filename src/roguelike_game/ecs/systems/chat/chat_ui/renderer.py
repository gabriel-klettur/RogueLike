...
from .text_wrap import wrap_text_small

logger = logging.getLogger(__name__)


class ChatUIRenderer:
    """Encapsula la lógica de renderizado del panel de chat."""

    def __init__(self, perf_log=None) -> None:
    ...
    def _project_root(self) -> Path:
        """Heurística robusta para encontrar la raíz del proyecto (con carpeta data/)."""
    ...
    def render(self, world, screen, camera) -> None:
        ...
        # Título dinámico (NPC -> nombre real)
        title_text = "Chat"
        try:
        ...
        # Estado online/offline (ya evaluado por otros sistemas via state.chat_llm_online)
        ...
        # Sincronizar preferencia de idioma desde memoria
        try:
            target_eid = getattr(state, 'chat_target_eid', None)
            if target_eid is not None and not getattr(state, 'chat_lang_preference', None):
        ...
        try:
            ...
            if getattr(state, 'chat_lang_rect', None) and state.chat_lang_rect.collidepoint(mx, my):
            ...
            if tooltip_lines:
                ...
                if tip_x + tip_w > sw2 - 6:
                    tip_x = sw2 - tip_w - 6
                if tip_y + tip_h > sh2 - 6:
                    tip_y = sh2 - tip_h - 6
                bg2 = pygame.Surface((tip_w, tip_h), pygame.SRCALPHA)
                bg2.fill((20, 20, 20, 220))
                pygame.draw.rect(bg2, (120, 120, 120), bg2.get_rect(), 1)
                screen.blit(bg2, (tip_x, tip_y))
                cy = tip_y + pad_tb
                for s in line_surfs:
        ...
        rh_size = btn_size
        rh_rect = pygame.Rect(
            panel_x + panel_w - pad - rh_size, panel_y + panel_h - pad - rh_size, rh_size, rh_size
        )
        try:
        ...
        pygame.draw.rect(
            screen, (180, 180, 255) if hovering_resize else (160, 160, 160), rh_rect, width=1
        )
        pygame.draw.line(
            screen, (200, 200, 255), (rh_rect.left + 3, rh_rect.bottom - 4), (rh_rect.right - 4, rh_rect.top + 3), 2
        )
        pygame.draw.line(
            screen, (200, 200, 255), (rh_rect.left + 5, rh_rect.bottom - 4), (rh_rect.right - 4, rh_rect.top + 5), 1
        )

        # Preparar controlador de input
        ...
        # Calcular altura del input envuelto
        ...
        wrapped_lines: list[tuple[str, str, bool, int]] = []
        for sender, text in raw_messages:
            ...
            first_width = max(0, msg_area_w - pref_w - 12)
            for i, seg in enumerate(wrap_text_small(small, text or "", first_width)):
        ...
        if show_scroll:
            ...
            pygame.draw.rect(
                screen, (160, 160, 160) if not hovering_thumb else (200, 200, 200), thumb_rect, border_radius=3
            )
            try:
