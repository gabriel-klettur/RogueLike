...
class CastOutlineRenderSystem:
    ...
    def update(self, world, screen: pygame.Surface, camera):
        ...
        outlines_list = world.components.get("CastOutlines", {})
        if not outlines and not outlines_list:
            return
        ...
        # 1) Renderizar múltiples outlines por entidad si existen en 'CastOutlines'
        for eid, lst in list(outlines_list.items()):
            pos = pos_map.get(eid)
            if pos is None:
                continue
            for outline in list(lst):
        ...
        # 2) Compatibilidad legacy: si no hay lista para la entidad, dibujar single 'CastOutline'
        for eid, outline in list(outlines.items()):
            if eid in outlines_list:
                continue  # evitar doble dibujado si ya hay lista
            pos = pos_map.get(eid)
            if pos is None:
                continue
            st = float(getattr(outline, "start_time", time.time()))
