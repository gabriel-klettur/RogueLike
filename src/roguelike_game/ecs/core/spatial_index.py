...
class SpatialIndex:
    ...
    def get_solid_tiles_for_rect(self, rect: pygame.Rect) -> list[pygame.Rect]:
        ...
        # Construir índice dinámico para edificios
        building_index: dict[tuple[int,int], list[pygame.Rect]] = {}
        for b in self.buildings:
            for tile_rect in b.collision_tiles:
                cell = (tile_rect.x // TILE_SIZE, tile_rect.y // TILE_SIZE)
                building_index.setdefault(cell, []).append(tile_rect)
        # Consultar colisiones de mapa y edificios
        for x in range(x1, x2 + 1):
            for y in range(y1, y2 + 1):
                result.extend(self._map_index.get((x, y), []))
                result.extend(building_index.get((x, y), []))
        return result
