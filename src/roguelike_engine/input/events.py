...
def handle_events(state, camera, clock, menu, map, entities):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            state.running = False

        elif event.type in (pygame.KEYDOWN, pygame.KEYUP):
            handle_keyboard(event, state, camera, clock, menu, entities)

        elif event.type in (pygame.MOUSEWHEEL, pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP):
            handle_mouse(event, state, camera, clock, map, entities)

    # Movimiento y laser continuo fuera del loop de eventos
    handle_continuous(state, camera, map, entities)