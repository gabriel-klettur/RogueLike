...
def spawn_entity(game, etype: str, tx: int, ty: int, player_stats: dict) -> None:
    ...
    if etype in player_stats:
        get_factory("player").create(game.ecs.ecs_world, tile_x=tx, tile_y=ty, class_player=etype)
    else:
        get_factory("monster").create(game.ecs.ecs_world, tile_x=tx, tile_y=ty, monster_type=etype)
