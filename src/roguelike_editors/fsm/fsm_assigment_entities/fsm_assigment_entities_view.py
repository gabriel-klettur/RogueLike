...
class FsmAssigmentEntitiesView:
    def __init__(self) -> None:
        self.panel_rect = None
        self.header_rect = None
        self.prev_rect = None
        self.next_rect = None
        self.value_col_x = 180
    ...
    def render(self, model, screen, *, anchor=(20, 120)):
        ...
        try:
            ...
            try:
                ...
                label = title_font.render("Entities Assignments", True, (240, 240, 240))
                surf.blit(label, (10, 6))
                # Prev/Next + current category
                tx, ty = 130, 6
                ...
                nx, ny = self.next_rect.x, self.next_rect.y
                pygame.draw.polygon(surf, (200, 200, 200), [(nx + 6, ny + 4), (nx + 14, ny + 10), (nx + 6, ny + 16)])
                cat = getattr(model, 'target_category', 'by_archetype')
                tgt_label = font.render(f"Category: {cat}", True, (230, 230, 230))
                ...
                th_val = font.render("Set ID", True, (220, 220, 220))
                ...
                for idx in range(start, end):
                    ...
                    t_key = font.render(row.key, True, color)
                    surf.blit(t_key, (10, row_y + 3))
                    v_text = row.value if row.value else "<unset>"
                    v_color = (220, 220, 220) if row.value else (180, 160, 160)
                    t_val = font.render(v_text, True, v_color)
                    surf.blit(t_val, (self.value_col_x, row_y + 3))
...
__all__ = ["FsmAssigmentEntitiesView"]
