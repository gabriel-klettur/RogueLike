...
def write_buildings_instances(data: List[Dict[str, Any]]) -> None:
                ...
                if len(candidates) > 1:
                    ...
                    for c in candidates:
                        try:
                            ...
                            ids.append(c.get('id'))
                    _log.debug(f"[BuildingsService][Dedup] key={k} candidates={ids} -> chosen={chosen_id}")
        except Exception:
            pass
    ...
    try:
        ...
        if added:
            _log.debug(f"[BuildingsService][Audit] Added IDs: {added}")
        if removed:
            _log.debug(f"[BuildingsService][Audit] Removed IDs: {removed}")
