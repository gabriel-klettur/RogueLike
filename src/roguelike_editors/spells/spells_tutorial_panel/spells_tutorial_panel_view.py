"""
Vista del panel de Tutorial (Spells Editor).
"""
import pygame
...
class SpellsTutorialPanelView:
    def __init__(self, model, editor_controller):
        self.model = model
        self.editor = editor_controller
        # Para alineación relativa al toolbar
    ...
    def _compute_position(self, screen: pygame.Surface, required_h: int) -> tuple[int, int]:
        ...
        # 1) Si el picker está visible, intentar colocarlo a la derecha del picker
        try:
            picker_rect = getattr(self.editor.view, 'grid_rect', None)
        except Exception:
            picker_rect = None
        if picker_rect is not None:
            x = min(max(margin, picker_rect.right + 12), max(margin, sw - self.width - margin))
            y = max(margin, min(picker_rect.top, sh - required_h - margin))
            return (x, y)
        # 2) Por defecto, alinear respecto del toolbar y del título
        try:
            title_rect = getattr(self.editor.view, 'title_rect', None)
        except Exception:
            title_rect = None
    ...
    def _measure_required_height(self, step: dict) -> int:
        ...
        if checklist:
            ...
            for item in checklist:
                ...
                ty += max(box_size, self.text_font.size(label)[1]) + 6
        btn_h = 32
    ...
    def render(self, screen: pygame.Surface) -> None:
        ...
        for key, rect in [('prev', prev_rect), ('next', next_rect), ('close', close_rect)]:
            ...
            ty_btn = rect.centery - text_surf.get_height() // 2
            screen.blit(text_surf, (tx, ty_btn))
        ...
        try:
            ...
            if isinstance(highlights, list):
                for hl in highlights:
                    ...
                    elif hl.get('kind') == 'panel':
                        which = hl.get('item')
                        if which == 'picker':
                            r = getattr(self.editor.model, 'panel_rect', None)
                        elif which == 'properties':
                            props_model = getattr(self.editor.spells_properties_controller, 'model', None)
                            r = getattr(props_model, 'panel_rect', None) if props_model else None
                        else:
                            r = None
                        if r:
                            self._draw_flash_highlight(screen, r.inflate(8, 8))
                    elif hl.get('kind') == 'add_remove':
                        which = hl.get('item')
                        try:
                            widget = getattr(self.editor.spells_add_remove_view, 'widget', None)
                            icon_rects = getattr(widget, 'icon_rects', {}) if widget else {}
                            r = icon_rects.get(which)
                        except Exception:
                            r = None
                        if r:
                            self._draw_flash_highlight(screen, r.inflate(8, 8))
