...
{
    /// <summary>
    /// Small help/tutorial overlay showing a list of hotkeys. Mirrors
    /// Python editors' tutorial panels
    /// (e.g. entities/panels/tutorial_panel.py).
    /// </summary>
    public static class TutorialOverlay
    {
        /// <summary>
        /// Builds a tutorial panel docked to the right side of the parent
        /// canvas. Returns the root GameObject (call SetActive to toggle).
        /// </summary>
        public static GameObject Build(Transform parent, string title, (string key, string action)[] lines)
        {
            var panel = UIPanel.Make("TutorialOverlay", parent,
            ...
            UILabel.MakeTitleBar(panel.transform, title, 26f);

            foreach (var (key, action) in lines)
            {
                var row = UIFactory.CreateUI("Row", panel.transform);
                row.AddComponent<LayoutElement>().preferredHeight = 20f;
                var hlg = row.AddComponent<HorizontalLayoutGroup>();
                hlg.spacing = 6f; hlg.childForceExpandWidth = false;
                hlg.childControlWidth = true; hlg.childControlHeight = true;

                var chip = UIFactory.CreateUI("Key", row.transform);
                chip.AddComponent<LayoutElement>().preferredWidth = 80f;
                var chipImg = chip.AddComponent<Image>();
                chipImg.color = UITheme.ACCENT_BG;
                var kt = UILabel.AddCenteredText(chip.transform, key, 11f,
                    FontStyles.Bold, UITheme.ACCENT);
                kt.characterSpacing = 0f;

                var actGo = UIFactory.CreateUI("Act", row.transform);
