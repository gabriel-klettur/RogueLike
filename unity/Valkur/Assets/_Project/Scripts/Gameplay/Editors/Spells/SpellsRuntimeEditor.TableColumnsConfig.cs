...
{
    /// <summary>
    /// Spells Editor — column visibility popup for the Table tab.
    ///
    /// Lets the user pick which SpellDefinition columns are rendered in the table.
    /// Reasons to hide columns:
    ///  • Narrow workflows only need a handful of fields visible at a time.
    ///  • Wide tables make horizontal scrolling exhausting.
    ///
    /// Persistence
    /// ───────────
    ...
    public partial class SpellsRuntimeEditor
    {
        ...
        private Toggle[]   _columnTogglesByIndex;  // indexed by SpellTableColumns.All
        private TextMeshProUGUI _columnsCountLabel; // counter inside popup footer
        ...
        /// <summary>
        /// Refresh the "N/M visible" counter on the table config bar.
        /// Called after every toggle so the bar reflects state in real time.
        /// </summary>
        internal void RefreshColumnsCountLabel()
        {
            if (_uiRefs.SpellsColumnsCfgLabel == null) return;
            int total   = SpellTableColumns.All.Count;
            int visible = total - _hiddenColumns.Count;
            _uiRefs.SpellsColumnsCfgLabel.text = $"Cols: {visible}/{total}";
        }
        ...
        private void BuildColumnsConfigPopup()
        {
            ...
            var rootGo = EditorUIHelpers.CreateUI("SpellsColumnsConfigPopup", _canvas.transform);
        ...
        private void BuildColumnsPopupRows(Transform parent)
        {
            ...
            SpellColumnCategory? prevCategory = null;
        ...
        private static void BuildColumnsPopupSectionLabel(Transform parent, SpellColumnCategory cat)
        {
            ...
            tmp.color      = SpellTableColumns.CategoryColor(cat);
            tmp.characterSpacing = 1.5f;
            tmp.margin     = new Vector4(2f, 0f, 0f, 0f);
        }

        private Toggle BuildColumnsPopupRow(Transform parent, SpellTableColumn col)
        {
            ...
            stripeGo.AddComponent<Image>().color = SpellTableColumns.CategoryColor(col.Category);
        ...
        private void UpdateColumnsCountLabelPopup()
        {
        ...
        private void OnColumnVisibilityToggled(SpellTableColumn col, bool visible)
        {
            if (col == null) return;
        ...
        /// <summary>
        /// Reset to the canonical defaults: TypeSpecific columns hidden, all others visible.
        /// </summary>
