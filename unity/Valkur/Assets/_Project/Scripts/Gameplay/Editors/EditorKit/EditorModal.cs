...
{
    /// <summary>
    /// Lightweight screen-space modal for runtime editors.
    /// Mirrors Python editors' add/remove/confirm/form dialogs (e.g. entities/panels/add_remove).
    ///
    /// Usage:
    ///   EditorModal.Confirm(canvas.transform, "Delete?", "This action cannot be undone.",
    ///       onOk: () => DoDelete(),
    ///       onCancel: null);
    ///
    ///   EditorModal.Prompt(canvas.transform, "New name:", "default",
    ///       onOk: val => rename(val),
    ///       onCancel: null);
    ///
    ///   EditorModal.Form(canvas.transform, "Add entity",
    ///       new [] { EditorModal.FormField.Text("Name", ""),
    ///                EditorModal.FormField.Int("HP", 10),
    ///                EditorModal.FormField.Dropdown("Type", new[]{"A","B"}, 0) },
    ///       result => { ... });
    /// </summary>
    public static class EditorModal
    {
        private const int MODAL_SORTING_ORDER = 30000;
            ...
            Action<string> onOk, Action onCancel = null)
        {
            ...
            var go = Show(parentCanvas, title, null, body =>
            {
                var iGo = EditorUIHelpers.CreateUI("Input", body);
                ...
                var textGo = EditorUIHelpers.CreateUI("Text", iGo.transform);
                EditorUIHelpers.StretchFill(textGo);
                var text = textGo.AddComponent<TextMeshProUGUI>();
                text.fontSize = 13f; text.color = EditorUIHelpers.TEXT_PRIMARY;
        ...
        {
            var modal = EditorUIHelpers.CreateUI("__EditorModal", parentCanvas);
            ...
            EditorUIHelpers.MakeTitleBar(card.transform, title ?? "", 28f);
            if (!string.IsNullOrEmpty(body))
            {
                var lbl = EditorUIHelpers.AddLabel(card.transform, body, 13f, TextAlignmentOptions.Left);
                lbl.color = EditorUIHelpers.TEXT_PRIMARY;
                lbl.GetComponent<LayoutElement>().preferredHeight = -1f;
            ...
            // Buttons row
            var row = EditorUIHelpers.CreateUI("Row", card.transform);
            ...
            if (showCancel)
            {
                EditorUIHelpers.MakeButton(row.transform, "Cancel", () =>
                {
                    if (modal != null) UnityEngine.Object.Destroy(modal);
                    done?.Invoke(false);
                });
            }
            EditorUIHelpers.MakeButton(row.transform, "OK", () =>
            {
                if (modal != null) UnityEngine.Object.Destroy(modal);
                done?.Invoke(true);
            });
        ...
        {
            var wrap = EditorUIHelpers.CreateUI("Field_" + f.Key, parent);
            ...
            var lbl = EditorUIHelpers.AddLabel(wrap.transform, f.Key + ":", 12f, TextAlignmentOptions.Left);
            ...
            {
                case FieldKind.Bool:
                    var tGo = EditorUIHelpers.CreateUI("Toggle", wrap.transform);
                ...
                case FieldKind.Dropdown:
                    var dGo = EditorUIHelpers.CreateUI("Dropdown", wrap.transform);
                    dGo.AddComponent<LayoutElement>().preferredWidth = 220f;
                    var dImg = dGo.AddComponent<Image>();
                    dImg.color = EditorUIHelpers.BG_SURFACE;
                    ...
                    break;
                default:
                    var iGo = EditorUIHelpers.CreateUI("Input", wrap.transform);
                    iGo.AddComponent<LayoutElement>().preferredWidth = 220f;
                    var iBg = iGo.AddComponent<Image>();
                    iBg.color = EditorUIHelpers.BG_SURFACE;
