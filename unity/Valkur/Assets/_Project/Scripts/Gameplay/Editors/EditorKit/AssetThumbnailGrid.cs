...
{
    /// <summary>
    /// Reusable thumbnail picker grid for runtime editors.
    /// Mirrors Python editors' asset grids (entities/panels/properties/asset_grid.py).
    /// Adds a search box, scroll view and a flexible number of columns.
    /// </summary>
    public sealed class AssetThumbnailGrid : MonoBehaviour
    {
        ...
        {
            var root = EditorUIHelpers.CreateUI(name, parent);
            ...
            var (scroll, content) = EditorUIHelpers.MakeScrollView(root.transform, "Items");
            scroll.horizontal = false; scroll.vertical = true;
            // Replace VLG with GridLayout on content
        ...
        public void Rebuild()
        {
            ...
            foreach (var e in _all)
            {
                ...
                var (btn, icon, label) = EditorUIHelpers.MakeSlotButton(_content, e.Label ?? e.Id,
        ...
        private void UpdateButtonVisual(int index)
        {
            ...
            if (img == null) return;
            // The child Entry for this button is by positional index among filtered entries;
            // SelectionChanged passes the Entry; for visual we use the SelectedId.
            bool selected = IsSelectedAt(index);
            img.color = selected ? EditorUIHelpers.SLOT_SELECTED : EditorUIHelpers.SLOT_BG;
        }
