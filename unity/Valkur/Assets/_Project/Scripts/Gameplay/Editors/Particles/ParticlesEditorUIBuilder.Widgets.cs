...
namespace Valkur.Gameplay.VFX
{
    ...
    {
        // Floating panel factory — same shape as Buildings/Entities/FSM builders.
            ...
            System.Action onClick, out TextMeshProUGUI tmp)
        {
            ...
            tmp           = AddCenteredText(go.transform, label, 10f, FontStyles.Bold, TEXT_PRIMARY);
            tmp.alignment = TextAlignmentOptions.Center;
            return img;
        }

        private static Image AddDangerBtn(Transform parent, string label, float height,
            System.Action onClick, out TextMeshProUGUI tmp)
        {
            var go = CreateUI($"Danger_{label}", parent);
            go.AddComponent<LayoutElement>().preferredHeight = height;
