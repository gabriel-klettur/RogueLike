...
// Registered per test assembly, in each Tests/<Mode>/<Root>/AssemblyInfo.cs, as
// [assembly: SelectableResetTestAction]. An assembly-level NUnit action reaches only the
// assembly that declares it, so every test assembly carries the line;
// TestLayoutConventionTests fails the one that forgets. NUnit fans BeforeTest out to each
// test in turn because the attribute targets ActionTargets.Test.
namespace Valkur.Tests.Support
{
    ...
    /// capacity before every test of each assembly that registers it. Prevents the
