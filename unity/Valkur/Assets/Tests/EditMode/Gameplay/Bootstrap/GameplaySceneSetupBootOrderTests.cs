    ...
    public class GameplaySceneSetupBootOrderTests
    {
        ...
        public void EnsureTileEditor_Precedes_LoadWorldProgressively_InStart()
        {
            ...
            Assert.IsTrue(File.Exists(scriptPath),
                $"GameplaySceneSetup.Sequence.cs not found at expected path: {scriptPath}");

            // Comment lines are dropped before the search. The sequence file documents
            // this very constraint above the step it constrains, so a raw scan finds the
            // explanation before the code and would go on passing for the wrong reason —
            // or start failing the day somebody rewords the paragraph.
