...
namespace Valkur.Tests.EditMode.Editors.ParticlesEditor
{
    ...
    public class ParticleCopyOnPlaceTests
    {
        ...
        public void TearDown()
        {
            ...
            // The loader test spawns emitters through ParticleInstancesLoader, which parents them
            // under a container it only creates in Start — and Start never runs in Edit Mode, so
            // they land at the scene root and survive the loader's own destruction. Left alive,
            // every later fixture that saves by FindObjectsOfType<PersistedParticleInstance>
            // counts one placement too many (it surfaced when the persistence fixtures started
            // running after this one).
            foreach (var identity in Object.FindObjectsOfType<PersistedParticleInstance>(includeInactive: true))
                if (identity != null) Object.DestroyImmediate(identity.gameObject);
