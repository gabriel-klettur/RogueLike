...
namespace Valkur.Tests.EditMode.Game.UI
{
    ...
    /// "Choosing 'Salir' from the pause menu must persist the current run
    ///  before returning to the main menu, so the player can pick 'Continuar'
    ///  on the next launch."
    ///
    /// Two layers of protection:
    ///   1. A behavioural test invokes ExecutePause("Salir") via reflection
    ...
    ///      "Salir" branch contains a QuickSave call. This catches accidental
    ...
    public class PauseMenuExitSavesTests
    {
        ...
        [Test]
        public void PauseMenuActions_SalirBranch_ContainsQuickSave()
        {
            ...
            // Find the "Salir" case and capture everything until the next
            // case label or the closing of the switch (best-effort: match
            // until the next "case " or "}" at column 16+).
            int salirIdx = source.IndexOf("case \"Salir\":", System.StringComparison.Ordinal);
            Assert.Greater(salirIdx, -1,
                "PauseMenuUI.Actions.cs must contain a 'Salir' case in ExecutePause");
            ...
            int windowEnd  = System.Math.Min(source.Length, salirIdx + 1500);
            string window  = source.Substring(salirIdx, windowEnd - salirIdx);
            ...
            Assert.IsTrue(window.Contains("QuickSave"),
                "The 'Salir' branch of ExecutePause must call SaveService.QuickSave " +
                "before SceneTransitionManager.LoadScene(\"MainMenu\"). " +
                "Otherwise the player's run is silently lost when exiting through " +
                "the pause menu and 'Continuar' will never appear in the main menu.");
        ...
        [Test]
        public void ExecutePause_Salir_DoesNotThrow_AndAttemptsSave()
        {
            ...
            // Look up _pauseOptions to find the "Salir" index dynamically
            ...
            int salirIdx = System.Array.IndexOf(opts, "Salir");
            Assert.Greater(salirIdx, -1, "Pause menu must have a 'Salir' entry");
            ...
            try
            {
                execute.Invoke(menu, new object[] { salirIdx });
            }
            catch (TargetInvocationException tie)
            {
                ...
                Assert.IsTrue(isSceneRelated,
                    $"ExecutePause('Salir') threw an unexpected exception before " +
