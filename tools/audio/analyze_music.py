...
a JSON report consumed by ``patch_audio_catalog_bpm.py`` to patch
``AudioCatalogSO`` track entries with ``bpm`` and ``key`` metadata.

Default behaviour scans the Valkur Unity music folder
(``unity/Valkur/Assets/_Project/Audio/Music``) and writes the report to
``tools/cache/audio/music_analysis.json``.
...
Usage (from repo root, with venv active)::
    ...
    python tools/audio/analyze_music.py --root path/to/music --out report.json --force

Then run ``python tools/audio/patch_audio_catalog_bpm.py`` to apply the
analysis to the Unity AudioCatalog .asset.
"""
...
def default_output() -> Path:
    repo = Path(__file__).resolve().parents[2]
    return repo / "tools" / "cache" / "audio" / "music_analysis.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Output JSON path (default: tools/cache/audio/music_analysis.json).",
    )
    parser.add_argument(
