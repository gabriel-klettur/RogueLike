Quick Valkur project status — git state, Unity console health, last test summary, migration progress at a glance. No arguments.

Produce a one-screen status report. Be concise.

## Gather

In parallel (independent tool calls in one batch):

1. **Git state** (via `run_commands`)
   - `git status` (snapshot of changes)
   - `git log -5 --oneline` (recent commits)

2. **Unity console**
   - `unityMCP__refresh_unity(scope="scripts", mode="if_dirty", wait_for_ready=true)`
   - `unityMCP__read_console(action="get", types=["error","warning"], page_size="20", format="plain")`

3. **Migration roadmap snapshot**
   - Read first 60 lines of `.github/MIGRATION_GUIDE.md` for phase totals.
   - Read `.github/MIGRATION_GUIDE.md` "Open Work" section.
