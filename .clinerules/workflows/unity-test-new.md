...
3. Decide:
   - Root = the highest production layer the test uses: `Core`, `Data`, `Infrastructure`, `UIKit`, `Gameplay`, `UI` (each is its own assembly). A runtime editor → `Assets/Tests/EditMode/Editors/<Name>Editor/`; `Scripts/Editor/` → `Assets/Tests/EditMode/EditorTools/<Folder>/`.
   - Feature folder = the production folder with its assembly prefix stripped; scene-dependent tests → `Assets/Tests/PlayMode/<Root>/<Feature>/`. Reserved names use the alias table in the skill.
...
5. Test names: `Subject_Scenario_Outcome`. Reflection via `TestReflection`; categories via `TestCategories`.
