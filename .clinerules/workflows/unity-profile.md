Capture a Unity profiling snapshot via MCP — frame timing across the three axes (CPU main, CPU render, GPU), top Update markers, and GC allocation rate. No arguments; the user should be in Play Mode in the scene/context they want profiled.
...
```csharp
// unityMCP__execute_code(action="execute", code=...)
return new {
...
Issue a single `unityMCP__execute_code` that enables and samples a known marker set. The data is from the **previous** frame; if all are zero, call once more (one frame later) for real numbers.
...
- After collecting, suggest adopting the `performance-optimizer` role (`.clinerules/agents/performance-optimizer.md`) for next steps if the bottleneck is significant.
