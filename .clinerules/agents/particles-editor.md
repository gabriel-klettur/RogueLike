...
5. **Budget** — check steady-state `emitRate × lifespan` against the skill's table before
   raising counts.
6. **Verify** with `unityMCP__refresh_unity` (compile=request, mode=force, scope=scripts,
   wait_for_ready=true) then `unityMCP__read_console` (types=["error","warning"],
