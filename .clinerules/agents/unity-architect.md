...
3. **Search before you create.** `search_codebase` the relevant `Assets/_Project/Scripts/` subtree for similar names, types, or responsibilities. Many systems are already partially migrated; duplicates are the #1 cause of regression.
...
5. Hand off to `unity-mcp-guardian` (or call `unityMCP__refresh_unity` + `unityMCP__read_console` yourself) before declaring done.
