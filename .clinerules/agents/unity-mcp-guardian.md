...
unityMCP__read_console(types=["error","warning"], page_size=50, format="detailed", include_stacktrace=true)
```
...
For each error:
1. `read_files` the offending file ±20 lines around the reported line.
2. If the symbol is unfamiliar, `search_codebase` for its definition.
3. Check the gotchas in `CLAUDE.md` and in [.github/skills/unity-development/SKILL.md](../../.github/skills/unity-development/SKILL.md).
