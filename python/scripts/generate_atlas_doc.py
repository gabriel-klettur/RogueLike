...
ACTIONS_PATH = Path(__file__).resolve().parents[1] / "data" / "cache" / "unity_asset_actions.json"
AUDIT_PATH = Path(__file__).resolve().parents[1] / "data" / "cache" / "unity_asset_audit.json"
...
w("> (`unity/Valkur/Assets/_Project/Art/`), NO sobre `python/assets/`.")
...
w()
w("> Datos generados automaticamente por `python/scripts/unity_asset_audit.py`.")
w()
w("---")
w()
...
w("> Los archivos de `python/assets/` NO se tocan.")
w()
...
w("---")
w()
w("*Documento generado automaticamente por `python/scripts/generate_atlas_doc.py`")
