...
def test_chat_router_handles_no_target(monkeypatch):
    # Evitar efectos de UI
    monkeypatch.setattr(router_system, 'push_bubble', lambda *a, **k: None, raising=True)

    # Worker y colaboradores falsos para no tocar hilos ni E/S
    class FakeWorker:
        _inst = None
        def __init__(self):
            self.submitted = []
    ...
    # Estado mínimo: chat abierto y sin target
    msgs = []
    class State:
        chat_open = True
        chat_messages = []
    ...
    # Debe generar un mensaje de error por falta de target
    assert any(who == 'NPC' and 'No tengo a nadie' in text for who, text in msgs)
