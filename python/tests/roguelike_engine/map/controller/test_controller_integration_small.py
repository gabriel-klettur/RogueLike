...
def test_build_map_integration_small(monkeypatch):
    class Settings:
        zone_width = 6
        zone_height = 4
        global_width = 12
        global_height = 8
        dungeon_connect_side = "bottom"
        additional_zones = { }
        zone_offsets = {'lobby': (3, 2), 'dungeon': (3, 6)}
        @staticmethod
        def _dynamic_offsets():
            return {'lobby': (3, 2), 'dungeon': (3, 6)}
    ...
    m = mc.build_map("test_world")

    assert m.name == "test_world"
    assert isinstance(m.metadata, dict) and 'lobby_offset' in m.metadata
