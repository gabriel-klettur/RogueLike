"""
Vista del panel de Tutorial (Map Editor).
"""
import pygame
...
class MapTutorialPanelView:
    ...
    def _compute_position(self, screen: pygame.Surface, required_h: int) -> tuple[int, int]:
        """
        Posiciona el panel del tutorial:
        - Por defecto a la derecha del toolbar, debajo del título.
        - Si no hay toolbar aún, acoplar al margen derecho con límites de pantalla.
        """
        margin = 16
        sw, sh = screen.get_size()

        # Intentar obtener rect del título desde MapEditorView
        def get_title_rect():
            try:
                r = getattr(self.editor_view, 'title_view', None)
                if r and hasattr(r, 'widget') and getattr(r.widget, 'rect', None):
                    return r.widget.rect
                # Algunos títulos devuelven el rect en render; intentar caché privada
                return getattr(self.editor_view, '_last_title_rect', None)
            except Exception:
                return None

        title_rect = get_title_rect()

        # Alinear a la derecha del toolbar, debajo del título
        if title_rect is not None and self.toolbar_view is not None and hasattr(self.toolbar_view, 'widget'):
            try:
                ...
                pass
        # Fallback al margen superior derecho
    ...
    def render(self, screen: pygame.Surface) -> None:
        ...
        if checklist:
            ty += self.spacing
            box_size = 16
            done_set = getattr(self.model, 'checklist_done_by_step', {}).get(idx, set())
            for item in checklist:
                label = item.get('label', '')
                iid = item.get('id')
        ...
        try:
            ...
            if isinstance(highlights, list):
                for hl in highlights:
                    ...
                    done_set2 = getattr(self.model, 'checklist_done_by_step', {}).get(idx, set())
                    if hide_if and (hide_if & done_set2):
                        continue
                    if deps and not deps.issubset(done_set2):
                        continue
                    ...
                    elif hl.get('kind') == 'editor_zone':
                        which = hl.get('which', 'selected')
                        r = None
                        if which == 'selected':
                            r = getattr(self.editor_view, '_last_selected_zone_rect', None)
                        if r:
