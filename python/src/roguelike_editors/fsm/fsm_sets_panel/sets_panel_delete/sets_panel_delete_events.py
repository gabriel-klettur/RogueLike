...
LOGGER = logging.getLogger("roguelike_editors.fsm.fsm_sets_panel.delete.events")


class SetsPanelDeleteEventHandler:
    def handle_button_click(self, parent_controller, index: int) -> bool:
        ...
        try:
            ...
            LOGGER.exception("[SetsPanelDelete] button click failed for index=%s: %s", index, ex)
        return True

    def handle_modal_event(self, parent_controller, event) -> bool:
        """Handle events while the confirmation modal is visible.
        Consumes interactions: left click on Yes/No and keys (Enter/Y, Esc/N).
        """
        try:
        ...
        rect = getattr(parent_controller.view, 'panel_rect', None)
        if not rect:
            return False
...
__all__ = ["SetsPanelDeleteEventHandler"]
