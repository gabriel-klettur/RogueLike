...
class SetsPanelDeleteView:
    """View responsible for rendering the delete confirmation modal.
    Stores the screen-space rects of the dialog and its buttons for hit-testing.
    """
    ...
    def render_modal(self, delete_model, screen, panel_rect) -> None:
        ...
        try:
            ...
            try:
                ...
                msg = getattr(delete_model, 'confirm_text', '') or 'Confirm?'
                lines = []
                words = msg.split()
                line = ''
                max_w = dw - 20
