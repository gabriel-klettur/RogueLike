...
LOGGER = logging.getLogger("roguelike_editors.fsm.fsm_graph_panel.tools.mark_ini.controller")


class MarkIniController:
    """Optional controller for the MarkIni tool.
    ...
    def activate(self, *, panel_controller, tool_model=None, tool_view=None) -> None:
        ...
        if self._tool_view is not None:
            try:
                ...
                LOGGER.exception("[MarkIniController] failed to apply config to view")
        LOGGER.debug("[MarkIniController] activated")

    def deactivate(self) -> None:
        LOGGER.debug("[MarkIniController] deactivated")
        self._panel = None
        self._tool_model = None
        self._tool_view = None


__all__ = ["MarkIniController"]
